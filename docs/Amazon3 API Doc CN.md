...
* 用户需要为API-KEY分配权限, 权限分为以下四种：
  ...
  * **TRANSFER** 权限允许用户调用划转接口, 来实现母账户和子账户之间的资产划转,

  * **WITHDRAWAL**  权限允许用户调用提现接口.
...
对API接入端的所有HTTP请求都需要身份验证和鉴权。用户可以通过创建API密钥来获取access-key和secret-key。access-key用于身份标识，secret-key用于数据签名。应将以下标头添加到所有HTTP请求中：

| Key                | Value                  | Description                                      |
| ------------------ | ---------------------- | ------------------------------------------------ |
| x-access-key       | < API-KEY >            | access-key 作为API接入时的身份标识               |
| x-access-sign      | < signatureOfRequest > | 基于请求内容的 Hash 值得出的数字签名或消息验证码 |
| x-access-timestamp | < timeOfRequest >      | 请求发起时的时间戳(毫秒级)                       |
| x-access-version   | < versionOfRequest >   | 签名协议的版本号，默认为 : 1.                    |

### 1.2.2 签名流程

**Example:**   
* 参与签名的字段包含header中的x-access-key、x-access-timestamp、x-access-version以及body中的所有字段, 按照字典序序列化为JsonString进行签名.
...
* 当收到429时，您有义务退后，不要向API发送垃圾邮件。
* 反复违反速率限制和/或在收到429后未能退出将导致自动IP禁止（HTTP状态418）。
* 对IP禁令进行跟踪，并对屡犯者延长期限，从2分钟到2天不等。
...
所有响应信息都采用JSON标准格式。包含三个字段，即“错误代码”、“错误代码”和“数据”。这些特定的业务数据包含在“数据”字段中。
...
```json
{
    ...
    "error_message":"",     // 错误描述
    "data":{
        "withdraw_order_id":"w00000001"  // 提现订单号
    }
}
```

### 2.4.3 交易账户与存管账户间的划转（需要“TRANSFER” 权限）
...
### 2.4.4 主/子交易账户之前的划转（需要“TRANSFER” 权限）
    ...
    "data":[
        {
            ...
            "high":"10",                    // 最高价格
            "low":"10",                     // 最低价格
            "volume_in_base":"100",         // 数量
            "start_time":1646213700000,     // 开始时间
            "end_time":1646213760000        // 结束时间
        }
    ]
}
```
...
### 3.1.1 WebSocket API 流控
* WebSocket连接每秒最多有5条传入消息
* 超出限制的连接将被断开；反复断开连接的IP地址可能会被禁止
* 单个连接最多可以侦听1024个流
...
- 一个API密钥最多可以创建10个长会话。例如，订阅交易数据允许API Key最多创建10个会话。
- 最多可以将5个长会话链接到一个IP地址。例如，市场数据推送不需要API密钥，一个IP地址最多可以链接5个会话。
...
{
    "id": 1,                          // 消息ID
    "result": null,                   // 当订阅部分成功时, result返回已成功订阅的主题
    "error_code": "0000"              // 错误码
}
```
    ...
    "data":[
        {
            ...
            "chg_vol":"0",              // 24小时变化数量
            "last_price":"3",           // 最后一笔价格
            "last_price_valuation":"0", // 最后一笔价格估值
            "volume_in_base":"2"        // 24小时交易数量
        }
    ]
}
```
